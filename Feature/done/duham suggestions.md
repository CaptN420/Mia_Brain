...
# The first line of defense in the workflow should be an AI orchestrator that checks the code against the first worker script and determines if it's capable of catching issues, and if there are missing issues, they should be added to the worker script, which will continually improve the script. Same for the thinker.

# Fix these issues one sub-agent at a time. Verify that the first subagent has completed it's task properly before dispatching the next one, and so on.
